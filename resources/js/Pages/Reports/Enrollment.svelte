...
<div class="flex flex-col m-4 sm:pl-14" id="report">
  <div class="bg-gray-200 rounded-sm shadow-md p-4 mb-2 flex flex-col">
    ...
    <div class="flex justify-center items-center flex-col">
      <h1 class="text-xl text-center font-bold">Enrollment Summary</h1>
      <p class="text-xs">{semester}, A.Y. {school_year}</p>
    </div>
  </div>
  <div class="-m-1.5 overflow-x-auto">
    <div class="p-1.5 min-w-full inline-block align-middle">
      {#each (report ?? []).sort((a, b) => {
        let comp = a.teacher.last_name.localeCompare(b.teacher.last_name);

        if(comp === 0) {
          comp = a.teacher.first_name.localeCompare(b.teacher.first_name);
        }
