...
<script>
  ...
  function updateHours() {
    for(let [key, value] of Object.entries(studentAttendanceData)) {
      if(studentAttendanceData[key]['status'] === 'absent') {
        studentAttendanceData[key]['hours'] = $form.hours;
      }
    }
  }
...
<div class="flex flex-col sm:pl-14 m-4">
  <fieldset class="grid gap-6 rounded-lg border p-4 mb-4">
    ...
    <div class="space-y-3">
      ...
      <div class="relative">
        <input type="number" bind:value={$form.hours} oninput={(e) => updateHours(e.target.value)} id="hours" class="peer p-4 block w-full border-gray-200 rounded-lg text-sm placeholder:text-transparent focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none focus:pt-6 focus:pb-2 [&:not(:placeholder-shown)]:pt-6 [&:not(:placeholder-shown)]:pb-2 autofill:pt-6 autofill:pb-2" placeholder="********">
